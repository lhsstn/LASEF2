...
DATASET_CONFIGS = {
    ...
    "mgsm": {
        "type": "mgsm",
        "col_map": {"question": "question", "answer": "answer"},
    },
    "default": {
        "type": "default",
        "col_map": {"question": "question", "answer": "answer", "id": "id"},
...
def parse_args():
    ...
    parser.add_argument(
        "--n_shot", type=int, default=0, help="Number of few-shot examples for solving"
    )
    ...
    parser.add_argument(
        "--langs",
        type=str,
        default="ta,kn,my,km,am,yo,si,gu,ne,uz,ky,ceb,eu,gn,hy,jv,ka,kk,ku,lo,mg,ml,mn,mr,mt,or,pa,ps,qu,sd,so,su,tg,ug",
    )
...
def get_dataset_config(dataset_name: str) -> dict:
    ...
    if "polymath" in name:
        return DATASET_CONFIGS["polymath"]
    if "aime" in name:
        return DATASET_CONFIGS["aime25"]
    if "math-500" in name:
        return DATASET_CONFIGS["math500"]
    if "mgsm" in name:
        return DATASET_CONFIGS["mgsm"]
    ...
    if modes_raw.strip().lower() == "all":
        ...
        for lang in all_langs:
            ...
            if lang != "en":
                ...
                modes.append(
                    f"en-{lang}-nonskeleton"
                )  # english question, native reasoning
        return modes
    normed = []
...
def run_skeleton_multiturn(
    args, llm, tok, config, gen_sp, sk_sp, trans_sp, load_targets, langs
):
    ...
    with open(args.output, "w", encoding="utf-8") as f_out:
        for target in load_targets:
            ...
            try:
                ds = (
                    load_dataset(args.dataset, name=curr_name, split=curr_split)
                    if curr_name
                    else load_dataset(args.dataset, split=curr_split)
                )
                ...
                for batch in tqdm(
                    ...
                    if args.translate_q:
                        ...
                        for i, t in enumerate(batch):
                            if t["lang"] != "en":
                                trans_prompts.append(
                                    build_translation_prompt(
                                        tok, t["question"], t["lang"], "en"
                                    )
                                )
                                trans_indices.append(i)

                        if trans_prompts:
                            trans_outputs = llm.generate(
                                trans_prompts, trans_sp, use_tqdm=False
                            )
                    ...
                    sk_prompts = [
                        build_skeleton_prompt(
                            tok, "en" if args.translate_q else t["lang"], q
                        )
                        for t, q in zip(batch, questions_to_use)
                    ]

                    if args.lora:
                        sk_outputs = llm.generate(
                            sk_prompts,
                            sk_sp,
                            use_tqdm=False,
                    ...
                    for batch_idx, (t, q, sks) in enumerate(
                        zip(batch, questions_to_use, skeletons_list)
                    ):
                        for sk_idx, sk in enumerate(sks):
                            ...
                            sol_prompts.append(
                                build_solver_prompt(
                                    tok, q, sk, t["lang"], force_english=force_en
                                )
                            )
                    ...
                    if sol_prompts:
                        ...
                        for (batch_idx, sk_idx), out in zip(
                            sol_prompt_mapping, sol_outputs
                        ):
                            if out.outputs:
                                sol_texts[batch_idx].extend(
                                    [o.text.strip() for o in out.outputs]
                                )
                                skeletons[batch_idx].append(
                                    skeletons_list[batch_idx][sk_idx]
                                )
                    ...
                    if args.translate_cot:
                        ...
                        if cot_trans_prompts:
                            cot_trans_outputs = llm.generate(
                                cot_trans_prompts, trans_sp, use_tqdm=False
                            )
        ...
        for target in load_targets:
            ...
            try:
                ...
                for batch in tqdm(
                    ...
                    if args.translate_q:
                        ...
                        for i, t in enumerate(batch):
                            if t["question_language"] != "en":
                                trans_prompts.append(
                                    build_translation_prompt(
                                        tok, t["prompt"], t["question_language"], "en"
                                    )
                                )
                                trans_indices.append(i)
                    ...
                    for t, q in zip(batch, questions_to_use):
                        force_en = (
                            args.translate_cot and t["reasoning_language"] != "en"
                        )
                    ...
                    sol_texts = [
                        [o.text.strip() for o in out.outputs] if out.outputs else []
                        for out in sol_outputs
                    ]
                    ...
                    if args.translate_cot:
                        ...
                        for i, (t, sol) in enumerate(zip(batch, sol_texts)):
                            if t["reasoning_language"] != "en" and sol:
                                cot_trans_prompts.append(
                                    build_translation_prompt(
                                        tok, sol[0], "en", t["reasoning_language"]
                                    )
                                )
                                cot_trans_indices.append(i)
...
def main():
    ...
    if config["type"] == "aime25":
        ...
        sk_sp = SamplingParams(
            temperature=eff_temp, max_tokens=args.max_tokens, n=16, top_p=eff_top_p
        )
    else:
        gen_sp = SamplingParams(
            ...
            top_k=args.top_k,
            top_p=eff_top_p, n=args.rollout)
        sk_sp = SamplingParams(
            temperature=eff_temp,
            max_tokens=args.max_tokens,
            top_k=args.top_k,
