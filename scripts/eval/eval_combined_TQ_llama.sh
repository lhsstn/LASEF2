...
# Datasets
# ==============================
MGSM=limhyeonseok/mgsm-translated
...
run_inference() {
    ...
    local OUTPUT="$PRJ_PATH/data/results/llama/${DATASET_NAME}/${MODEL_NAME}-${METHOD}${SUFFIX}-Google-transQ${ROLLOUT_SUFFIX}.jsonl"
    ...
    CUDA_VISIBLE_DEVICES=$CUDA_VISIBLE_DEVICES python3 "$PRJ_PATH/scripts/src/eval_combined_TQ_llama.py" \
        --method "$METHOD" \
        --model "$MODEL_PATH" \
        --dataset "$DATASET" \
        --output "$OUTPUT" \
...
ALL_LANGS="zh,es,ko,th,sw,te,bn"
